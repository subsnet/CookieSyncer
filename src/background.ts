...
const AUTO_MERGE_INTERVAL = 60 * 60 * 1000;  // 1 hour

/* tslint:disable no-console */

// Auto Merge
const autoMerge = _.debounce(async () => {
  console.log('自动合并运行中');
  await gist.reload();
...
setInterval(autoMerge, AUTO_MERGE_INTERVAL);
chrome.windows.onCreated.addListener(autoMerge);
chrome.tabs.onCreated.addListener(autoMerge);
...
async function filterDomain(type: 'autoPush' | 'autoMerge'): Promise<Array<[string, AutoConfiguration]>> {
  ...
  if (type === 'autoPush') {
    ...
    const settings = await auto.getAll();
    ...
    const domains = await gist.getDomainList();
    return domains.map((domain) => {
      const config: AutoConfiguration = settings.get(domain) || {
        autoPush: false,
        autoMerge: true,
        autoPushName: [],
      };
      const result: [string, AutoConfiguration] = [domain, config];
      return result;
    }).filter(([domain, config]) => config.autoMerge);
  }
